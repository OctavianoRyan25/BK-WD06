...
@section('content')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2">Dokter</span>
                </div>
            </li>
        </ol>
    </nav>
    <div class="container mx-auto w-full px-4 py-8">
        <div class="row">
            <div class="col-md-12">
                <h1 class="font-bold text-3xl">Dokter</h1>
            </div>
        </div>
        ...
        <div class="w-full md:w-fit flex justify-between md:items-center gap-4 my-3 mt-3">
            <button id="addUser" href="{{ route('admin.dokter.create') }}" onclick="openModal()"
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-[#003d7a]">
                    <tr>
                        ...
                        <th class="py-3 px-6 text-center text-sm font-medium text-white uppercase tracking-wider">Poliklinik
                        </th>
                    ...
                    @else
                        @foreach ($dokters as $index => $dokter)
                            <tr class="hover:bg-gray-100 transition duration-200">
                                <td class="py-2 px-3 text-center">{{ $dokters->firstItem() + $index }}</td>
                                <td class="py-2 px-3 text-center">{{ $dokter->nama }}</td>
                                <td class="py-2 px-3 text-center">{{ $dokter->alamat }}</td>
                                <td class="py-2 px-3 text-center">{{ $dokter->no_hp }}</td>
                                <td class="py-2 px-3 text-center">{{ $dokter->poli->nama_poli }}</td>
                                <td class="py-2 px-3 text-center gap-2">
                                    <div class="flex justify-center space-x-2">
                                        ...
                                        <a href="{{ route('admin.dokter.update', $dokter->id) }}"
        ...
        <div id="createDokterModal"
            ...
            <div class="bg-white p-6 rounded-lg shadow-lg transform transition-all duration-300 scale-90 opacity-0 w-full sm:w-3/4 lg:w-6/12"
                ...
                <h2 class="text-xl font-bold mb-4 text-start">Tambah Dokter</h2>
                <hr class="bg-black mb-2" />
                <form id="dokterForm" action="{{ route('admin.dokter.store') }}" method="POST">
                    @csrf
