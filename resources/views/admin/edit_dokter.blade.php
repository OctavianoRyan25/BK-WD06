        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2">Tambah Dokter</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="container w-full">
        <div class="px-6 py-10 bg-white shadow-lg rounded-lg mt-3 mx-3">
            <div class="mb-6 text-center">
                <h1 class="text-3xl font-bold text-gray-800">Update Data Dokter</h1>
                <p class="text-gray-600 text-sm mt-2">Isi informasi dokter dengan lengkap untuk menambah melakukan baru.</p>
            </div>
            {{-- looping error --}}
            @if ($errors->any())
            ...
            @endif
            <form action="{{ route('admin.dokter.handleupdate', $dokter->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="nama" class="block text-sm font-semibold text-gray-700">Nama Dokter</label>
                    ...
                    <div>
                        <label for="id_poli" class="block text-sm font-semibold text-gray-700">Poli</label>
                        <select name="id_poli" id="id_poli"
                            class="mt-2 focus:ring-teal-500 focus:border-teal-500 block w-full shadow-sm text-sm border-gray-300 rounded-lg py-2 px-3">
                            <option value="{{ $dokter->poli->id }}" selected>{{ $dokter->poli->nama_poli }}
                            </option>
                    ...
                    <div>
                        ...
                        <input type="tel" name="no_hp" id="no_hp" autocomplete="off"
                            placeholder="Masukkan nomor telepon" value="{{ $dokter->no_hp }}"
                            class="mt-2 focus:ring-teal-500 focus:border-teal-500 block w-full shadow-sm text-sm border-gray-300 rounded-lg py-2 px-3 placeholder-gray-400"
                            required>
                    </div>
                </div>
