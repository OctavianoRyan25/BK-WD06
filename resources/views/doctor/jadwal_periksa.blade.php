...
@section('title', 'Poliklinik - Jadwal Periksa')
@section('content')
    ...
    <div class="container mx-auto w-full px-4 py-8">
        <div class="row">
            <div class="col-md-12">
                <h1 class="font-bold text-3xl">Jadwal Periksa</h1>
            </div>
        </div>
                ...
                <thead class="bg-[#003d7a]">
                    <tr>
                        ...
                        <th class="py-3 px-6 text-center text-sm font-medium text-white uppercase tracking-wider">Nama
                            Dokter</th>
                        <th class="py-3 px-6 text-center text-sm font-medium text-white uppercase tracking-wider">Hari
                        </th>
                        <th class="py-3 px-6 text-center text-sm font-medium text-white uppercase tracking-wider">Jam Mulai
                        </th>
                    ...
                    @else
                        @foreach ($jadwals as $index => $jadwal)
                            <tr class="hover:bg-gray-100 transition duration-200">
                                <td class="py-2 px-3 text-center">{{ $jadwals->firstItem() + $index }}</td>
                                <td class="py-2 px-3 text-center">{{ $jadwal->dokter->nama }}</td>
                                <td class="py-2 px-3 text-center">{{ $jadwal->hari }}</td>
                                <td class="py-2 px-3 text-center">{{ $jadwal->jam_mulai }}</td>
                                <td class="py-2 px-3 text-center">{{ $jadwal->jam_selesai }}</td>
                                @if ($jadwal->status == 'Aktif')
                                    <td class="py-2 px-3 text-center text-green-500">{{ $jadwal->status }}</td>
                                @else
                                    <td class="py-2 px-3 text-center text-red-500">{{ $jadwal->status }}</td>
                                @endif
                                <td class="py-2 px-3 text-center gap-2">
                                    <div class="flex justify-center space-x-2">
                                        <button class="text-blue-600 updateEvent transition duration-300 hover:scale-110"
                                            onclick="openReadModal({{ $jadwal->id }}, '{{ $jadwal->dokter->nama }}', '{{ $jadwal->hari }}', '{{ $jadwal->jam_mulai }}', '{{ $jadwal->jam_selesai }}', '{{ $jadwal->status }}')">
                                        ...
                                        <button class="text-blue-600 updateEvent transition duration-300 hover:scale-110"
                                            onclick="openUpdateModal({{ $jadwal->id }}, '{{ $jadwal->dokter->id }}','{{ $jadwal->dokter->nama }}', '{{ $jadwal->hari }}', '{{ $jadwal->jam_mulai }}', '{{ $jadwal->jam_selesai }}', '{{ $jadwal->status }}')">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                        ...
                                        <form action="{{ route('dokter.jadwal-periksa.delete', $jadwal->id) }}"
        ...
        @if ($jadwals->isNotEmpty())
            <div id="updateJadwalModal"
                class="fixed inset-0 hidden items-center justify-center bg-gray-900 bg-opacity-50 flex transition-opacity duration-300">
                <div class="bg-white p-6 rounded-lg shadow-lg transform transition-all duration-300 scale-90 opacity-0 w-full sm:w-3/4 lg:w-6/12"
                    id="updatePesertaContent">
                    <h2 class="text-xl font-bold mb-4 text-start">Jadwal Obat</h2>
                    <hr class="bg-black mb-2" />
                    <form id="obatForm" action="{{ route('dokter.jadwal-periksa.update', $jadwal->id) }}"
                        method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="flex flex-wrap">
                            <div class="mb-6 px-3 w-full lg:w-1/2">
                                <label for="dokter_id" class="block text-sm font-medium text-gray-700">Nama Dokter</label>
                                <input type="hidden" name="dokter_id" id="updateDokterId"
                                    class="mt-2 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                    required value="{{ auth()->guard('dokter')->user()->id }}" readonly>
                            ...
                            <div class="mb-6 px-3 w-full lg:w-1/2">
                                <label for="hari" class="block text-sm font-medium text-gray-700">Hari</label>
                                <select name="hari" id="updateHari"
                                    class="mt-2 block w-full border border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2"
                                    required>
                                    <option value="" disabled>Pilih Hari</option>
                                    <option value="Senin">Senin</option>
                        ...
                        <div class="flex flex-wrap">
                            <div class="mb-6 px-3 w-full lg:w-1/3">
                                <label for="jam_mulai" class="block text-sm font-medium text-gray-700">Jam Mulai</label>
                                <input type="time" name="jam_mulai" id="updateJamMulai"
                                    class="mt-2 block w-full border border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2"
                                    required>
                                <p class="text-red-500 text-sm mt-2 error" id="error-jam_mulai"></p>
                            </div>
                            <div class="mb-6 px-3 w-full lg:w-1/3">
                                <label for="jam_selesai" class="block text-sm font-medium text-gray-700">Jam
                                    Selesai</label>
                                <input type="time" name="jam_selesai" id="updateJamSelesai"
                                    class="mt-2 block w-full border border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2"
                                    required>
                                <p class="text-red-500 text-sm mt-2 error" id="error-jam_selesai"></p>
                            </div>
                    ...
                    <div class="flex justify-between gap-6 mt-6 px-3">
                        <button onclick="closePartialModal('updateJadwalModal')"
